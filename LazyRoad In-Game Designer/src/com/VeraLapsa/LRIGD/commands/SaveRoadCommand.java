 ...
 * @author VeraLapsa
 */
public class SaveRoadCommand extends CommandHandler {

    private Road road = null;

    public SaveRoadCommand(LazyRoadInGameDesigner plugin) {
        super(plugin);
    }
    ...
    public boolean perform(CommandSender sender, String[] args) {
        ...
        if (plugin.roadsetup.containsKey(sender.getName())) {
            switch (args.length) {
                case 1:
                    if (saveRoad(sender, args[0], false)) {
                        return true;
                    } else {
                        return false;
                    }
                case 2:
                    if (args[1].equalsIgnoreCase("true")) {
                        if (saveRoad(sender, args[0], true)) {
                            return true;
                        } else {
                            return false;
                        }
